...
// Re-export the trip schema for validation utilities
...
export function validateTripData(tripData: any, tripId: string): TripValidationResult {
  ...
  if (tripData.status === 'completed' && tripData.endDate > new Date()) {
    result.warnings.push('Trip marked as "completed" but end date is in the future');
  }

  if (tripData.status === 'planned' && tripData.startDate < new Date()) {
    result.warnings.push('Trip marked as "planned" but start date is in the past');
  }

  // Validate image paths
  if (tripData.headerImage && !tripData.headerImage.startsWith('/')) {
...
/**
 * Validates all trips in the collection
 */
export async function validateAllTrips(): Promise<TripValidationResult[]> {
  try {
    ...
    return [{
      tripId: 'collection',
      isValid: false,
      errors: [`Failed to load trips collection: ${error}`],
      warnings: []
    }];
  }
}
